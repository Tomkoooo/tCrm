import { connectDB, Product, StockLevel, Supplier, Warehouse } from '@crm/db';
import { availableQty } from '../logistics/stock-helpers';
import { getBulkAvailability } from '../logistics/availability';
...
export type InventoryDashboardOptions = {
  ...
  /** Limit stock/KPI rows to these warehouses (e.g. user scope). */
...
export async function getInventoryDashboardSummary(
  ...
  const lowStockItems = [...stockByProduct.entries()]
    .map(([productId, stock]) => {
      ...
      const available = availableQty({ onHand: stock.onHand, reserved: stock.reserved });
      return {
        productId,
        sku: product.sku,
        name: product.names?.hu ?? product.names?.en ?? product.sku,
        onHand: stock.onHand,
  ...
  const buildIds = builds.map((b) => b._id as mongoose.Types.ObjectId);
  const availability = await getBulkAvailability(buildIds);

  const buildsAvailability = builds
    .map((b) => {
      const avail = availability.get(String(b._id));
      return {
        productId: String(b._id),
        sku: b.sku,
        name: b.names?.hu ?? b.names?.en ?? b.sku,
        canBuild: avail?.canBuild ?? 0,
